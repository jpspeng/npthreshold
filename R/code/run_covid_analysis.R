...
for(marker in markers){
  ({
    ...
    marker_values_for_thresh <- na.omit(data_sub[[paste0(marker, "")]])
    min_thresholds <- sapply(union(variant_names, "Ancestral.Lineage"), function(vname) {
      min(na.omit(data_sub[[paste0(marker, "_", vname, "_1")]]))
    })


    # Only keep marker values for observations where an event occurs.

    all_thresholds <- sort(as.vector(na.omit(marker_values_for_thresh[data_sub[[event_type]] != 0])))
